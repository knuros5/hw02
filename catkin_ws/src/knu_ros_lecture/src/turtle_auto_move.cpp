...
doTranslation(ros::Publisher &pubTeleop, tf::Transform &initialTransformation, double dTranslation, double dTranslationSpeed, Vec3d &curPos, Vec3d &initPos, Vec3d &curRot, bool &flag, vector<Vec3d> &route, Mat &display)
{
	...
	while(ros::ok() && !bDone) {
		...
		printf("current position : %.3lf , %.3lf\n", curPos[0], curPos[1]);
		...
		if(fabs(dDistMoved) >= fabs(dTranslation)) {
			bDone = true;
			wayPoints.push_back(Vec3d(x,y,0));// save real destination point
			break;
		} else {
...
void
drawTrajectory(Mat &display, vector<Vec3d> &trajectory, Vec3d &curPos, double dMaxDist)
{
	Vec2i imageHalfSize = Vec2i(display.cols/2, display.rows/2);
...
int main(int argc, char **argv)
{
	...
	for(int i = 0; i < num; i++)
	{
		...
		doTranslation(pub, nextTransformation, distance, linear_vel, curPos, prePos, curRot, flag, route, display);
		prev_angle = angle;
